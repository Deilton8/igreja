...
<div class="max-w-3xl mx-auto mt-10">
    <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
        <div class="mb-6 flex items-center justify-between border-b pb-4">
            <div>
                <h2 class="text-2xl font-bold">✏️ Editar Publicação</h2>
                <p class="text-gray-500 text-sm">Ajuste as informações abaixo.</p>
            </div>
        ...
        <?php if (!empty($_SESSION['flash'])): $flash = $_SESSION['flash']; unset($_SESSION['flash']); ?>
            <div class="mb-4 p-3 rounded <?= isset($flash['success']) ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800' ?>">
        ...
        <form method="POST" class="space-y-5">
            <div>
                ...
                <input type="text" name="titulo" required value="<?= htmlspecialchars($publicacao['titulo']) ?>" class="w-full p-3 border rounded-lg">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Conteúdo</label>
                <textarea name="conteudo" rows="8" class="w-full p-3 border rounded-lg"><?= htmlspecialchars($publicacao['conteudo']) ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="categoria" class="w-full p-2 border rounded-lg">
                        ...
                        <option value="noticia" <?= $publicacao['categoria'] === 'noticia' ? 'selected' : '' ?>>Notícia</option>
            ...
            <div>
                ...
                <input type="datetime-local" name="publicado_em" value="<?= $publicacao['publicado_em'] ? date('Y-m-d\TH:i', strtotime($publicacao['publicado_em'])) : '' ?>" class="w-full p-2 border rounded-lg">
            </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <?php foreach($midias as $m): ?>
                        <label class="block border rounded-lg p-2 cursor-pointer hover:shadow">
                            <input type="checkbox" name="midias[]" value="<?= $m['id'] ?>" class="mb-2" <?= in_array($m['id'], $midiasPublicacao ?? []) ? 'checked' : '' ?>>
                            ...
                            <?php elseif ($m['tipo_arquivo'] === 'video'): ?>
                                <video class="w-full h-28 object-cover rounded" muted><source src="/<?= $m['caminho_arquivo'] ?>"></video>
            ...
            <div class="flex items-center justify-between pt-6 border-t mt-6">
                ...
                <button type="submit" class="bg-gradient-to-r from-green-600 to-teal-600 text-white px-6 py-2.5 rounded-lg">Atualizar</button>
            </div>
        </form>
    </div>
</div>
