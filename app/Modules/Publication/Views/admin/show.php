...
<div x-data="mediaPreview(<?= htmlspecialchars(json_encode($publicacao['midias'])) ?>)" x-cloak
    ...
    <div class="space-y-3">
        <p><span class="font-semibold text-gray-700">ID:</span> <?= $publicacao['id'] ?></p>
        <p><span class="font-semibold text-gray-700">Título:</span> <?= htmlspecialchars($publicacao['titulo']) ?></p>
        <p><span class="font-semibold text-gray-700">Slug:</span> <?= htmlspecialchars($publicacao['slug']) ?></p>
        <p><span class="font-semibold text-gray-700">Conteúdo:</span>
            <?= nl2br(htmlspecialchars($publicacao['conteudo'])) ?></p>
        <p><span class="font-semibold text-gray-700">Categoria:</span> <?= $publicacao['categoria'] ?></p>
        <p><span class="font-semibold text-gray-700">Status:</span>
            <span
                class="px-2 py-1 rounded text-sm
                <?= $publicacao['status'] === 'publicado' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' ?>">
                <?= $publicacao['status'] ?>
            </span>
        </p>
        <p><span class="font-semibold text-gray-700">Publicado em:</span> <?= $publicacao['publicado_em'] ?></p>
    </div>
    ...
    <div class="mt-6 flex items-center justify-between">
        ...
        <a href="/admin/publicacoes" class="text-gray-600 hover:text-gray-800">Voltar</a>
    </div>
