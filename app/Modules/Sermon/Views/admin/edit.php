...
<div class="bg-white shadow rounded-lg p-6 max-w-2xl mx-auto">
    ...
    <form method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="titulo" value="<?= htmlspecialchars($sermao['titulo'] ?? '') ?>" required
        ...
        <div>
            ...
            <textarea name="conteudo" rows="6"
                class="w-full px-3 py-2 border rounded"><?= htmlspecialchars($sermao['conteudo'] ?? '') ?></textarea>
        </div>

        <div>
            <label class="block font-semibold mb-1">Pregador</label>
            <input type="text" name="pregador" value="<?= htmlspecialchars($sermao['pregador'] ?? '') ?>"
                class="w-full px-3 py-2 border rounded">
        </div>

        <div>
            <label class="block font-semibold mb-1">Data</label>
            <input type="date" name="data" value="<?= $sermao['data'] ?? date('Y-m-d') ?>" required
                class="w-full px-3 py-2 border rounded">
        </div>
        ...
        <div>
            ...
            <select name="status" class="w-full px-3 py-2 border rounded">
                <option value="rascunho" <?= ($sermao['status'] ?? '') === 'rascunho' ? 'selected' : '' ?>>Rascunho
                </option>
                <option value="publicado" <?= ($sermao['status'] ?? '') === 'publicado' ? 'selected' : '' ?>>Publicado
                </option>
            </select>
        </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <?php foreach ($midias as $m): ?>
                    <label class="flex flex-col items-center rounded p-2 cursor-pointer hover:shadow">
                        <input type="checkbox" name="midias[]" value="<?= $m['id'] ?>" <?= in_array($m['id'], $midiasSermao ?? []) ? 'checked' : '' ?> class="mb-2">
        ...
        <div class="flex justify-between pt-4">
            <button type="submit" class="bg-blue-600 text-white px-5 py-2 rounded hover:bg-blue-500">Atualizar
                Sermão</button>
